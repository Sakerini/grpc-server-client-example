...
//bi-directional stream -- server sends numbers to client and client to server
app.get("/exchangeNumbers", function(request, response) { 
    let numbers = {
        number: 10
    }
    var call = clientStub.exchangeNumbers(function(error, status) {
        if (error) {
            // returned error
            console.log("error");
            response.end("fail to send numbers")
            ...
            // everything recieved
            console.log("numbers sent" + status)
            response.end("numbers sent")
        }
    });
    ...
    call.on('data', function(number) {
        console.log("From server" + number.number)
    });
